...
void LogstashWriter::CheckResultHandler(const Checkable::Ptr& checkable, const CheckResult::Ptr& cr)
{
        ...
        if (!IcingaApplication::GetInstance()->GetEnablePerfdata() || !checkable->GetEnablePerfdata())
                return;
        /***********************************************************************************************/
		//Dictionary::Ptr dict_log = new Dictionary();
...
void LogstashWriter::SendMetric(const String& prefix, const String& name, double value, double ts)
{
        ...
        try {
				Dictionary::Ptr dic = new Dictionary(); 
				dic->Set("message1", "Hello from Icinga2");
				dic->Set("message2", "does this work?");
				//dic->Set("arrayInt", "/[\"djnjn\", 1, false]/");
				dic->Set("booelan", true);
				dic->Set("integer", 1);
			//	dic->Set("ArrayString",["rfrfgr", "heiII2"]);
String replace[] = {"{", "}", "\\n", "\\", "\"", "\"\\\"", "\\\"" };
	String s = JsonEncode(dic)+ "\n";
//	boost::replace_all(s, "\"/[", "[");	// "/[
//	boost::replace_all(s, "]/\"", "]");	// ]/"
/*	for(int i=0; i<4; i++){
		boost::replace_all(s, replace[i], "");
	}
	boost::replace_all(s, "\",\"", "\",\\n\"");
	boost::replace_all(s, "\\"", "\"");
	boost::replace_all(s, ",", " ");
	boost::replace_all(s, ":", "=>");*/
	//s+="\n";
	m_Stream->Write(&s[0],s.GetLength());

//////////////////////////////////////////////////////////////////
/*String test= "{\"KEY 1\":\"Some String\", \"KEY 2\": 0, \"KEY 3\": [\"String\", 0, true]}\n";
	m_Stream->Write(&test[0], test.GetLength() );*/
/////////////////////////////////////////////////////////////////
