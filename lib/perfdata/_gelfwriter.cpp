...
void GelfWriter::CheckResultHandler(const Checkable::Ptr& checkable, const CheckResult::Ptr& cr)
{
	...
	SendLogMessage(gelfObject(fields, GetSource(), ts));
}
...
String GelfWriter::gelfObject(const Dictionary::Ptr& fields, const String& source, double ts)
{
...
//////////////////////////////////////////////////////////////////////////////	
	...
	/*for (int i =0; i<(sizeof(from )/ sizeof(from[0])); i++){
		boost::replace_all(temp, from[i], to[i]);
	}*/
...
/*	std::vector<std::string> from, to;
	String file;
	std::ifstream openfile("/etc/icinga2/conf.d/pattern.txt");
	if(openfile.is_open()){
		while(!openfile.eof()){
			std::string temp1, temp2;
			getline(openfile, temp1, '~');
			take.push_back(temp1);
			getline(openfile, temp2, '~');
			make.push_back(temp2);
		}
		openfile.close();	
	}
	...
	for(int i=0; i<from.size(); i++){	
		boost::replace_all(gelfObj, from[i], to[i]);
	}*/
//////////////////////////////////////////////////////////////////////////////
	Log(LogDebug, "GelfWriter")<< "Original Object '" << gelfObj << "'.";
	return temp+"\n";
}
